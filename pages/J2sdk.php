...
<body>
            ...
            <div class="row page-titles mx-0">
                ...
                <div class="col p-md-0">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active"><a href="./J2sdk.php">J2sdk1.4.0-javax-swing</a></li>
                    </ol>
                </div>
            </div>
            ...
            <div class="container-fluid mt-3">
                ...
                <div class="row" style="margin-top: -35px;">
                    <div class="col-lg-6">
                        <div class="card">
                            <div class="card-body">
                                <h1 class="card-title" style="font-size: 16px;">Overall Clone Type</h1>
                ...
                <div class="row">
                    ...
                    <div class="col-lg-6">
                        <div class="card">
                            <div class="card-body">
                                <h4 class="card-title" style="font-size: 16px;">Runtime Performance of J2sdk1.4.0-javax-swing</h4>
    ...
    ***********************************-->
    ...
    <script>
        ...
        // Iterate over each line to aggregate data for J2SDK applications
        lines.forEach(line => {
            const [appName, , , cType, cCount] = line.split(',');

            // Check if the application is a J2SDK application and avoid duplicates
            if (appName.includes('J2sdk1.4.0-javax-swing')) {
                if (!chartData[cType]) {
                    chartData[cType] = 0;
                }
    ...
    <script>
        ...
        lines2.forEach(line => {
            ...
            // Check if the application is a J2SDK application
            if (appName.includes('J2sdk1.4.0-javax-swing')) {
    ...
    <script>
        ...
        // Parse CSV data from local storage
        const parsedDataJ2sdk = localStorage.getItem('uploadedCSV');
        const linesJ2sdk = parsedDataJ2sdk.split('\n').slice(1); // Remove header

        let labels = [];
        let type1_2Data = [];
        let type3_4Data = [];

        // Iterate over each line to accumulate J2sdk data
        linesJ2sdk.forEach(line => {
            // Split line and trim spaces
            const parts = line.split(',').map(part => part.trim());

            // Check if line has enough columns and the correct app name
            if (parts.length >= 11 && parts[0] === 'J2sdk1.4.0-javax-swing') {
                const type1_2 = parts[10];
                const type3_4 = parts[11];

                // Aggregate data only for J2sdk application
                if (!labels.includes('J2sdk1.4.0-javax-swing')) {
                    labels.push('J2sdk1.4.0-javax-swing');
                }
    ...
    <script>
        ...
        // Get parsed data from local storage for the second chart
        const parsedDataJ2sdk2 = localStorage.getItem('uploadedCSV');
        const linesJ2sdk2 = parsedData2.split('\n').slice(1); // Remove header

        // Initialize chart data object for the second chart
        const chartDataJ2Sdk = {
            'Pre-processing': 0,
            'Transformation': 0,
            'Parameterization': 0,
            'Categorization': 0,
        ...
        // Iterate over each line to aggregate data for the second chart
        linesJ2sdk2.forEach(line => {
            const [appName, , , , , preprcss, trans, prmtrzn, ctg, matchD] = line.split(',');

            // Check if the application is a J2SDK application
            if (appName.includes('J2sdk')) {
                // Aggregate data for Preprcss, Trans, Prmtrzn, Ctg, and MatchD
                chartDataJ2Sdk['Pre-processing'] = parseInt(preprcss);
                chartDataJ2Sdk['Transformation'] = parseInt(trans);
                chartDataJ2Sdk['Parameterization'] = parseInt(prmtrzn);
                chartDataJ2Sdk['Categorization'] = parseInt(ctg);
                chartDataJ2Sdk['Match Detection'] = parseInt(matchD);
            }
        });

        // Get chart labels and values for the second chart
        const chartLabelsJ2sdk = Object.keys(chartDataJ2Sdk);
        const chartDataValuesJ2sdk = Object.values(chartDataJ2Sdk);

        const data4 = {
            labels: chartLabelsJ2sdk,
            datasets: [{
                label: 'Runtime Performance',
                backgroundColor: 'rgba(255,231,0,1)', // Green
                borderColor: 'rgba(255,231,0,1)',
                borderWidth: 1,
                data: chartDataValuesJ2sdk
            }]
        };
            ...
            scales: {
                x: {
                    ...
                    ticks: {
                        ...
                        maxRotation: 10, // Rotate labels to 180 degrees
                        minRotation: 10 // Rotate labels to 180 degrees
                    }
                },
                y: {
