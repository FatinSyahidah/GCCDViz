...
<body>
            ...
            <div class="row page-titles mx-0">
                ...
                <div class="col p-md-0">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active"><a href="./E-jdtcore.php">Eclipse-jdtcore</a></li>
                    </ol>
                </div>
            </div>
            ...
            <div class="container-fluid mt-3">
                ...
                <div class="row">
                    ...
                    <div class="col-lg-6">
                        <div class="card">
                            <div class="card-body">
                                <h4 class="card-title" style="font-size: 16px;">Runtime Performance of Eclipse-jdtcore</h4>
    ...
    ***********************************-->
    ...
    <script>
        ...
        // Iterate over each line to aggregate data for EJdt applications
        lines.forEach(line => {
            const [appName, , , cType, cCount] = line.split(',');

            // Check if the application is a EJdt application and avoid duplicates
            if (appName.includes('Eclipse-jdtcore')) {
                if (!chartData[cType]) {
                    chartData[cType] = 0;
                }
    ...
    <script>
        ...
        lines2.forEach(line => {
            ...
            // Check if the application is a Eclipse-jdtcore application
            if (appName.includes('Eclipse-jdtcore')) {
    ...
    <script>
        ...
        // Parse CSV data from local storage
        const parsedDataEJdt = localStorage.getItem('uploadedCSV');
        const linesEJdt = parsedDataEJdt.split('\n').slice(1); // Remove header

        let labels = [];
        let type1_2Data = [];
        let type3_4Data = [];

        // Iterate over each line to accumulate EJdt data
        linesEJdt.forEach(line => {
            
            // Split line and trim spaces
            const parts = line.split(',').map(part => part.trim());

            // Check if line has enough columns and the correct app name
            if (parts.length >= 11 && parts[0] === 'Eclipse-jdtcore') {
                const type1_2 = parts[10];
                const type3_4 = parts[11];

                // Aggregate data only for J2sdk application
                if (!labels.includes('Eclipse-jdtcore')) {
                    labels.push('Eclipse-jdtcore');
                }
    ...
    <script>
        ...
        // Get parsed data from local storage for the second chart
        const parsedDataEJdt2 = localStorage.getItem('uploadedCSV');
        const linesEJdt2 = parsedData2.split('\n').slice(1); // Remove header
        ...
        // Iterate over each line to aggregate data for the second chart
        linesEJdt2.forEach(line => {
            const [appName, , , , , preprcss, trans, prmtrzn, ctg, matchD] = line.split(',');

            // Check if the application is a EJdt application
            if (appName.includes('Eclipse-jdtcore')) {
                // Aggregate data for Preprcss, Trans, Prmtrzn, Ctg, and MatchD
                chartDataEJdt['Pre-processing'] = parseInt(preprcss);
                chartDataEJdt['Transformation'] = parseInt(trans);
                chartDataEJdt['Parameterization'] = parseInt(prmtrzn);
                chartDataEJdt['Categorization'] = parseInt(ctg);
                chartDataEJdt['Match Detection'] = parseInt(matchD);
            }
        });

        // Get chart labels and values for the second chart
        const chartLabelsEJdt = Object.keys(chartDataEJdt);
        const chartDataValuesEJdt = Object.values(chartDataEJdt);

        const data4 = {
            labels: chartLabelsEJdt,
            datasets: [{
                label: 'Runtime Performance',
                backgroundColor: 'rgba(255,231,0,1)', // Green
                borderColor: 'rgba(255,231,0,1)',
                borderWidth: 1,
                data: chartDataValuesEJdt
            }]
        };
